...
public class PredictController {
    ...
    public String Predict(@RequestParam(name="date") String date,@RequestParam(name="check_image") String[] imgs , Model model){
        ...
        String t1 = imgs[0];
        String t2 = imgs[1];
        List<String> list = iRecord.search(t1,t2,date);
        if (list == null){
            return "index";
        }
        model.addAttribute("team1",list.get(0));
        model.addAttribute("team2",list.get(1));
        model.addAttribute("probability",list.get(2));
        String img_1 = map.get(list.get(0));
        String img_2 = map.get(list.get(1));
